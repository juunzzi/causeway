...
**PM2 앱 이름은 신경 쓰지 않아도 된다.** `bin/<앱>` 과 `scripts/restart.sh` 는 이름과 로그
경로를 `ecosystem.config.cjs` 에서 읽는다 — 거기만 맞으면 스크립트는 따라온다. 예전에는 각
파일이 이름을 따로 들고 있어서, 하나만 바꾸면 `pm2 restart --only <옛이름>` 이 **아무것도
재시작하지 않은 채 성공으로 끝나고** 뒤이은 부팅 판정만 "신호 없음"으로 실패했다. 지금은 그
경우 "PM2 에 그 앱이 없다"로 즉시 갈라진다.
...
### 자동 배포는 기본으로 붙어 있지 않다

배포 워크플로는 `.github/workflows/` 가 아니라 [`examples/deploy.yml`](../examples/deploy.yml)
에 있다. **그 자리에서는 실행되지 않는다** — GitHub 은 `.github/workflows/` 안의 파일만
워크플로로 인식하기 때문이다.

일부러 그렇게 뒀다. 이 봇은 당신 머신에서 돌고 GitHub 이 거기로 들어올 수 없어 배포에는
self-hosted 러너가 필요한데, 러너가 없는 레포에 워크플로를 두면 트리거를 꺼놔도 Actions 탭에
뜨고 누군가 실행하면 **큐에 걸린 채 영원히 대기한다**(실패로 표시되지도 않는다).

쓰려면 세 단계다 — 러너 등록 → `runs-on` 라벨과 체크아웃 경로 수정 → 제자리로 복사:

```bash
cp examples/deploy.yml .github/workflows/
```

러너는 **레포 밖 경로에 설치한다.** 러너 작업 디렉토리가 워킹트리를 더럽히면
`auto-update.sh` 의 "로컬 수정 있음" 게이트가 배포를 영구 보류시킨다.

배포의 실질은 워크플로가 아니라 `scripts/auto-update.sh` 에 있다(pull → 유휴 대기 → 재시작 →
부팅 판정 → 실패 시 롤백·격리). 러너 없이 쓰려면 그 스크립트를 인자 없이 실행해 폴링 모드로
돌려도 된다.
