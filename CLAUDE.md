...
- **`channels.yaml` 의 중복 검증은 `논리명` 과 `(ID, role)` 쌍 기준이다.** 서로 다른 role 이
  같은 채널 ID 를 가리키는 것은 허용된다 — 1인 운영 봇에서 ops·release 를 같은 DM 으로 두는
  구성이 정상이기 때문이다. 반대로 **같은 role 에 두 채널을 걸면 부팅이 죽는다**(어디로 보낼지
  모호해진다).
...
- **타이머에는 `createTicker` 를 건다. 맨 `tick` 을 직접 걸면 안 된다.** dedup_key 가 막는 것은
  잡이지 **이미 게시된 루트 메시지**가 아니다. 슬랙이 안 닿아 `postRoot` 가 몇 분씩 붙들리면
  그 사이 던져진 tick 들이 모두 "아직 잡이 없다"를 보고 각자 게시하고, 네트워크가 돌아오는
  순간 같은 루트가 무더기로 쏟아진다. 재진입 가드가 겹침 자체를 없앤다.
- **하루 한 번은 `oncePerDay: true`** 로 표현한다. 키에서 시각을 떼므로 cron 의 여러 시각이
  "또 돈다"가 아니라 **예비 시각**이 된다 — 먼저 성공한 하나가 그날 몫을 가져간다.
