...
이 봇의 값어치는 **당신 조직의 도구**에서 나온다. 붙이는 자리는 세 곳이다:

1. `src/mcp/<도구>.ts` — 도구 구현. 순수부(가드·요청 빌드·요약)와 부작용부를 파일 안에서 가른다.
2. `src/mcp/registry.ts` — 등록 배열에 한 줄. **이 배열이 곧 노출 경계**라 glob 자동발견은 쓰지 않는다.
3. `src/context.ts` — 배선 조건(env 유무 등) 판정 + 부팅 로그 한 줄.

경계는 프롬프트가 아니라 **입력 스키마**로 만든다. 예를 들어 조회 전용 도구라면 `method` 필드를
아예 두지 않는다 — "쓰기 하지 마"를 모델 순응도에 맡기지 않기 위해서다.
